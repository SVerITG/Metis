...
Generate a portable context brief that can be pasted into any AI (Gemini, GPT-4, another Codex session) or used to resume work on another device. Called when you hit a token limit, switch AI tools, or need to continue work elsewhere.
...
## CONTEXT FOR THIS AI
You are a research assistant continuing work started in Metis (a personal PKM system built on Codex). The user's Research Context (RC) is stored locally on OneDrive. You do not have access to the SQLite database or file tools in this session — rely on information above and what the user shares directly.
