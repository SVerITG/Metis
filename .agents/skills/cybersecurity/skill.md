...
When invoked as `/cybersecurity` from Codex:
...
## Reasoning
Cybersecurity operates automatically alongside internet-enabled agents (Librarian, News Radar, News Aggregator). Before any external call is made, three checks fire automatically via the pre-tool-use hook (`.Codex/hooks/pre-tool-use.mjs`):
