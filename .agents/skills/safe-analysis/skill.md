...
The safe way to use Metis on sensitive data — patient records, identifiable surveillance data, embargoed or unpublished datasets. **The raw data never enters a prompt.** Metis writes the analysis code, *you* run it on your own machine, and only derived, non-identifiable outputs (column names, value counts, summary tables, model results) come back. Codex reasons over the **shape** of your data, never the records.
