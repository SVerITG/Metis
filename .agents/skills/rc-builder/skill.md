...
When invoked as `/rc-builder` from Codex:
...
Scope:
- New specialist agents — produce the full agent folder (`skill.md`, `system-prompt.md`, `contract.md`, optional `output-spec.md` + `workflows.md`) plus a Codex skill stub in `metis/.Codex/skills/<slug>/skill.md`.
...
- **Migration notes** — anything an existing user must do (run setup-mcp.sh again, restart Codex Desktop, run a DB migration)
