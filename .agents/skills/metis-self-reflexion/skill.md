...
6. **Reimagine, don't only verify.** For every surface and subsystem, ask: *knowing what we know now — and that Codex keeps shipping (connectors, tools, scheduled tasks, the phone app) and that Metis must also serve non-Codex AI — would we build this the same way?* Capture forward-looking redesign recommendations.
...
| Anthropic docs — newest Codex connectors, tools, scheduled tasks, MCP primitives (sampling/elicitation), Codex mobile | input to the Reimagine stream |
...
Spawn as parallel subagents (Agent tool, `subagent_type="Explore"` for read/probe, `"Codex"` for synthesis). Brief every one with the skeptic framing: *ground every finding in a file:line / command output / DB query; run the external signal yourself; prefer "works" only after you have proven it; return `severity`, `evidence`, `acceptance_test`, and the `angle` you tested from.*
...
- **Codex is evolving** — connectors, tools, scheduled tasks, the mobile app, new MCP primitives (sampling, elicitation), computer use. Can this surface/subsystem integrate more deeply instead of reimplementing? (e.g. should the scheduler be a Codex scheduled task? should capture flow through the phone app? should memory use a native primitive?)
- **Metis is not Codex-only.** It is meant to serve other AI systems too. How would this integrate with non-Codex models / other agent runtimes / a provider-agnostic MCP layer? What is hard-coded to Codex that shouldn't be?
...
1. Personal memory file: `~/.Codex/projects/{hash}/memory/evaluations/YYYY-MM-DD_self-reflexion.md` (self-contained, readable months later). Use `ls ~/.Codex/projects/` to find the hash.
