...
{
  "command": "wsl.exe",
  "args": ["-d", "Ubuntu-24.04", "-e", "/home/<user>/.local/share/metis-mcp/run.sh"]
}
```
...
The `.venv` was inside `08_system/mcp-server/` — on OneDrive. Two failure modes:
...
**Fix applied:** Created `/home/<user>/.local/share/metis-mcp/run.sh` which sets
...
**Fix applied:** Deleted `02_agents/ruflo-reference/` entirely.
...
| File | Purpose |
|---|---|
| `08_system/mcp-server/setup.sh` | One-command setup for a new computer |
| `08_system/SETUP.md` | Human-readable setup instructions |
| `08_system/session-log.md` | Running log of discoveries (this session was its first entry) |
...
3. If on a new computer: follow `08_system/SETUP.md`.
