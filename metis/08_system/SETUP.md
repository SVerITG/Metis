...
```bash
cd "/mnt/c/Users/<user>/OneDrive/Documents/7. Software/PKM/metis/08_system/mcp-server"
bash setup.sh
```
...
| File access warning for `swarm-runner-reference` on startup | Agent directory existed but was completely empty; `list.files()` hit an inaccessible OneDrive placeholder inside it | Deleted the empty directory |
