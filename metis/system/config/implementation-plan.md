...
- [~] **XL — Windows .exe installer**  
  ...
  *Code written — needs the researcher to run `ISCC.exe metis-setup.iss` on a Windows machine with Inno Setup 6.*
...
- [x] **L1** (S) — `pdf_chunks` + `pdf_index_state` tables added to `system/installer/schema.sql` — DONE 2026-05-14
- [x] **L2** (M) — `vec_pdf_chunks` vec0 virtual table (sqlite-vec, 768-dim) — DONE 2026-05-14
- [x] **L3** (L) — `build_pdf_knowledge_db()` MCP tool: pypdf → 3200-char chunks → batch nomic-embed → SQLite — DONE 2026-05-14
- [x] **L4** (M) — `search_pdf_knowledge(query, top_k, domain_filter)` MCP tool: vec0 ANN search — DONE 2026-05-14
- [x] **L5** (S) — `get_pdf_index_stats()` MCP tool — DONE 2026-05-14
- [x] **L6** (M) — Dashboard: PDF search bar + stats panel on Knowledge tab; `/api/partial/knowledge/pdf-search`, `/api/partial/knowledge/pdf-stats`, `/api/knowledge/build-index` (background trigger) — DONE 2026-05-14
- [x] **L7** (M) — `build_knowledge_db.py` standalone CLI (`--library-dir`, `--db`, `--domain`, `--force`, `--quiet`) — DONE 2026-05-14
- [x] **L8** (S) — `metis-setup.iss` Step 2 post-install: silent background DB build for full/standard editions — DONE 2026-05-14
- [ ] **L9** (S) — **Distributable DB export**: `export_knowledge_db.py` — exports `pdf_chunks` + `vec_pdf_chunks` to standalone `knowledge_db.sqlite` for optional ~150 MB download (deferred — run indexer first, export after)

**Status 2026-05-14:** Indexing 338 PDFs running in background (open-access-books/ + papers/). Expected ~20-40 min. Scanned PDFs (no text layer) will show ✗ no text — these need OCR or manual extraction.
