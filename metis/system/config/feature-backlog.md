...
- [ ] **Release Orchestrator agent (`/release-coordinator`)** — Coordinates the full Metis multi-repo release workflow. Design:
  ...
  - Pre-publish checklist runs automatically: scans all staged files for personal data (names, API keys, email addresses, article titles, institution names, DB paths, `<user>`, `itg.be`, the researcher's name), blocks if found
