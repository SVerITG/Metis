...
async def metis_identity(request: Request):
    ...
    name = prefs.get("display_name") or "the researcher"
    ...
    return HTMLResponse(
        ...
        f'<div style="font-family:var(--m-display);font-style:italic;font-size:13px;color:var(--m-muted);">researcher@example.org · {runs} agent runs total</div>'
        f'</div>'
