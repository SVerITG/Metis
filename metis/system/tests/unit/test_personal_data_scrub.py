...
_PATTERNS: list[Pattern] = [
    ...
    Pattern("first_name",       re.compile(r"\bStan\b", re.IGNORECASE),
            "First name (the researcher)"),
    Pattern("family_name",      re.compile(r"the researcher", re.IGNORECASE),
            "Family name"),
    Pattern("username",         re.compile(r"<user>", re.IGNORECASE),
            "Username (<user>)"),
    Pattern("initials",         re.compile(r"S\.V\."),
            "Initials S.V."),
    Pattern("partial_name",     re.compile(r"the researcher V\b"),
            "Partial name (the researcher V)"),
    # Email
    Pattern("email_domain",     re.compile(r"@itg\.be"),
            "Institutional email domain @example.org"),
    Pattern("email_prefix",     re.compile(r"<user>@"),
            "Email prefix <user>@"),
    # Institution
    Pattern("institute_full",   re.compile(r"your institution"),
            "Full institution name"),
    Pattern("institute_abbrev", re.compile(r"\bITG\b"),
            "Institution abbreviation ITG (standalone)"),
    # Local paths
    Pattern("wsl_path",         re.compile(r"/mnt/c/Users/<user>"),
            "WSL personal path"),
    Pattern("win_path_bs",      re.compile(r"C:\\\\Users\\\\<user>"),
            "Windows backslash path"),
    Pattern("win_path_fs",      re.compile(r"C:/Users/<user>"),
    ...
    Pattern("sqlite_personal",  re.compile(r"metis\.sqlite.*<user>|<user>.*metis\.sqlite"),
...
_ALLOWED_PATTERNS = {
    ...
    "researcher@example.org",  # email only in .env.example placeholder — filtered by file type
}
...
def is_allowlisted(line: str, match: re.Match) -> bool:
    ...
    if "SVerITG" in surrounding and "<user>" not in surrounding.lower():
        return True
...
def test_claude_md_no_real_name(metis_root: Path):
    ...
    for pat in [re.compile(r"\bStan\b"), re.compile(r"the researcher"), re.compile(r"<user>")]:
        m = pat.search(text)
...
def test_git_log_no_personal_data(repo_root: Path):
    ...
    personal_pats = [
        ...
        re.compile(r"the researcher", re.IGNORECASE),
        re.compile(r"<user>", re.IGNORECASE),
        re.compile(r"@itg\.be"),
        re.compile(r"your institution"),
    ]
    violations = []
    for msg in messages:
...
def test_pyproject_no_personal_author(repo_root: Path):
    ...
    assert "<user>" not in text, "pyproject.toml contains personal username"
    assert "@example.org" not in text, "pyproject.toml contains institutional email"
    assert "the researcher" not in text.lower(), "pyproject.toml contains family name"
