...
news_server <- function(id, paths) {
  moduleServer(id, function(input, output, session) {
    ...
    # ── AI & Tools news section ──────────────────────────────────────
    output$ai_news_section <- renderUI({
      ai_briefs <- tryCatch(
        db_table(paths, paste(
          "SELECT title, summary, brief_date, signal_strength, COALESCE(source_url,'') AS source_url",
          "FROM news_briefs WHERE LOWER(domain) IN ('ai','ai & tools','ai tools')",
          "ORDER BY brief_date DESC LIMIT 8"
        )),
        error = function(...) data.frame()
      )

      if (!nrow(ai_briefs)) return(NULL)
      ...
      div(class = "news-ai-card",
        ...
        lapply(seq_len(nrow(ai_briefs)), function(i) {
          b   <- ai_briefs[i, ]
          sig <- switch(b$signal_strength,
                        high   = "🔴",
                        medium = "🟡",
                        "⚪")
          div(class = "news-ai-item",
            div(style = "display:flex; justify-content:space-between; align-items:baseline;",
              if (nzchar(b$source_url)) {
                tags$a(href = b$source_url, target = "_blank",
                  style = "color:#7dd3fc; font-weight:500; text-decoration:none;",
                  paste(sig, b$title))
              } else {
                tags$span(style = "font-weight:500;", paste(sig, b$title))
              },
              tags$span(style = "font-size:0.72rem; color:#94a3b8;", b$brief_date)
            ),
            if (nzchar(b$summary))
              tags$p(style = "font-size:0.78rem; color:#cbd5e1; margin:0.2rem 0 0;", b$summary)
          )
        })
      )
    })
