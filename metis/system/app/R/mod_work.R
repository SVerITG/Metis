# mod_work.R
# Work — project and task management with kanban board + table views.
...
work_ui <- function(id) {
  ...
  tagList(
      ...
      card(
        ...
        card_body(
          ...
          p(class = "text-muted", style = "font-size:0.82rem;",
            icon("rocket"), " Launcher — optional shortcut to open this project"),
          layout_columns(
            col_widths = c(4, 8),
            selectInput(ns("project_launcher_type"), "Launcher",
                        choices = c(
                          "None"           = "",
                          "RStudio"        = "rstudio",
                          "VS Code"        = "vscode",
            ...
            textInput(ns("project_launcher_path"), "Path / URL",
    ...
    # ── Projects table with archive/remove actions ──────────────────
    card(
      card_header("All projects"),
      card_body(class = "card-scroll", uiOutput(ns("projects_table")))
    )
  )
}

work_server <- function(id, paths) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$save_project, {
      ...
      insert_project(paths, input$project_title, input$project_domain,
                     ...
                     launcher_type = input$project_launcher_type,
                     launcher_path = input$project_launcher_path)
    ...
    # ── Launch project ───────────────────────────────────────────────
    observeEvent(input$launch_project, {
      req(input$launch_project)
      act  <- input$launch_project
      ltype <- act$launcher_type
      lpath <- act$launcher_path

      if (is.null(ltype) || !nzchar(ltype) || is.null(lpath) || !nzchar(lpath)) {
        showNotification("No launcher configured for this project.", type = "warning")
        return()
      }

      result <- tryCatch({
        switch(ltype,
          rstudio   = system2("cmd", c("/c", "start", "", shQuote(lpath)),
                               wait = FALSE, stderr = NULL, stdout = NULL),
          vscode    = system2("cmd", c("/c", "code", shQuote(lpath)),
                               wait = FALSE, stderr = NULL, stdout = NULL),
          explorer  = system2("explorer", shQuote(lpath),
                               wait = FALSE, stderr = NULL, stdout = NULL),
          claude    = {
            system2("cmd", c("/c", "start", "", "claude:"),
                     wait = FALSE, stderr = NULL, stdout = NULL)
            0L
          },
          0L
        ...
        showNotification(paste("Launch failed:", conditionMessage(e)), type = "error")
        -1L
      })

      if (!is.null(result) && result == 0L) {
        showNotification(paste("Launching", ltype, "\u2014", basename(lpath)), type = "message")
      }
    }, ignoreNULL = TRUE)
    ...
    output$projects_table <- renderUI({
      ...
      projects <- tryCatch(
        db_table(paths,
          "SELECT project_id, title, domain, status, priority, next_step,
                  COALESCE(launcher_type,'') AS launcher_type,
                  COALESCE(launcher_path,'') AS launcher_path
           FROM projects ORDER BY created_at DESC"),
        error = function(...) data.frame()
      )
      if (!nrow(projects)) return(p(class = "status-info", "No projects yet."))
      ...
      tagList(lapply(seq_len(nrow(projects)), function(i) {
        p <- projects[i, ]
        is_archived <- p$status == "archived"

        # Launcher button (only if configured)
        launch_btn <- if (!is_archived && nzchar(p$launcher_type) && nzchar(p$launcher_path)) {
          licon <- launcher_icon_map[p$launcher_type]
          llabel <- launcher_label_map[p$launcher_type]
          if (is.na(licon)) licon <- "rocket"
          if (is.na(llabel)) llabel <- p$launcher_type
          tags$button(
            class = "btn btn-xs btn-launch",
            style = "background:#174c4f; color:#fff; border:none; padding:2px 8px;",
            onclick = sprintf(
              "Shiny.setInputValue('%s', {launcher_type:'%s', launcher_path:'%s'}, {priority:'event'})",
              ns("launch_project"), p$launcher_type, gsub("'", "\\\\'", p$launcher_path)
            ),
            tagList(icon(licon), paste0("  ", llabel))
          )
        } else NULL

        div(style = if (is_archived) "opacity:0.6; margin-bottom:0.4rem;" else "margin-bottom:0.4rem;",
          div(style = "display:flex; align-items:center; gap:0.5rem; padding:0.5rem 0; border-bottom:1px solid var(--metis-border);",
            div(style = "flex:1;",
              tags$strong(p$title),
              tags$span(style = "margin-left:0.4em; font-size:0.75rem; color:var(--metis-text-muted);",
                        paste0("[", p$domain, "] ", p$status, " · ", p$priority))
            ),
            if (!is.null(launch_btn)) launch_btn,
            if (is_archived) {
              tags$button(
                class = "btn btn-xs btn-outline-secondary",
                onclick = sprintf("Shiny.setInputValue('%s','%s',{priority:'event'})",
                                  ns("unarchive_project"), p$project_id),
                "Un-archive"
              )
            } else {
              div(style = "display:flex; gap:0.25rem;",
                tags$button(
                  ...
                  onclick = sprintf("Shiny.setInputValue('%s','%s',{priority:'event'})",
                                    ns("archive_project"), p$project_id),
                  "Archive"
                ),
                tags$button(
                  class = "btn btn-xs btn-remove",
                  onclick = sprintf("Shiny.setInputValue('%s','%s',{priority:'event'})",
                                    ns("remove_project"), p$project_id),
                  "Remove"
                )
              )
            }
    ...
    # ── Archive / unarchive / remove project ────────────────────────
    observeEvent(input$archive_project, {
      req(input$archive_project)
      tryCatch({
        con <- connect_db(paths)
        on.exit(DBI::dbDisconnect(con), add = TRUE)
        DBI::dbExecute(con, "UPDATE projects SET status = 'archived' WHERE project_id = ?",
                       params = list(input$archive_project))
      }, error = function(...) NULL)
      refresh_trigger(refresh_trigger() + 1L)
    }, ignoreNULL = TRUE)

    observeEvent(input$unarchive_project, {
      req(input$unarchive_project)
      tryCatch({
        con <- connect_db(paths)
        on.exit(DBI::dbDisconnect(con), add = TRUE)
        DBI::dbExecute(con, "UPDATE projects SET status = 'active' WHERE project_id = ?",
                       params = list(input$unarchive_project))
      }, error = function(...) NULL)
      refresh_trigger(refresh_trigger() + 1L)
    }, ignoreNULL = TRUE)

    observeEvent(input$remove_project, {
      req(input$remove_project)
      pid <- input$remove_project
      proj <- tryCatch(
        db_table(paths, sprintf("SELECT title FROM projects WHERE project_id = '%s'", pid)),
        error = function(...) data.frame()
      )
      title_str <- if (nrow(proj)) proj$title[1L] else pid

      showModal(modalDialog(
        title = tagList(icon("triangle-exclamation"), " Remove project"),
        tagList(
          tags$p(sprintf("Remove '%s' from Metis?", title_str)),
          tags$p(style = "font-size:0.84rem; color:var(--metis-text-muted);",
                 "The project folder on disk will NOT be deleted — only the Metis record.")
        ),
        footer = tagList(
          modalButton("Cancel"),
          actionButton(ns("confirm_remove_project"), "Remove", class = "btn-danger")
        ),
        easyClose = FALSE
      ))
    }, ignoreNULL = TRUE)

    observeEvent(input$confirm_remove_project, {
      pid <- input$remove_project
      tryCatch({
        con <- connect_db(paths)
        on.exit(DBI::dbDisconnect(con), add = TRUE)
        DBI::dbExecute(con, "DELETE FROM tasks WHERE project_id = ?", params = list(pid))
        DBI::dbExecute(con, "DELETE FROM projects WHERE project_id = ?", params = list(pid))
      }, error = function(...) NULL)
      removeModal()
      refresh_trigger(refresh_trigger() + 1L)
    })
