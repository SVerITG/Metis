    ...
    card(
      ...
      card_body(
        div(class = "action-row",
          ...
          tags$div(style = "margin-left:auto; display:flex; align-items:center; gap:0.4rem;",
            checkboxInput(ns("show_archived"), "Show archived", value = FALSE),
            tags$span(style = "font-size:0.82rem; color:var(--metis-text-muted);",
                      uiOutput(ns("archived_count"), inline = TRUE))
          )
        ),
...
library_server <- function(id, paths) {
  moduleServer(id, function(input, output, session) {
    ...
    lib_archive_trigger <- reactiveVal(0L)      # refreshes archive overlay
    ...
    # ── Archive/remove handlers ──────────────────────────────────────
    observeEvent(input$archive_lib_item, {
      req(input$archive_lib_item)
      tryCatch(archive_library_item(paths, input$archive_lib_item), error = function(...) NULL)
      lib_archive_trigger(lib_archive_trigger() + 1L)
    }, ignoreNULL = TRUE)

    observeEvent(input$unarchive_lib_item, {
      req(input$unarchive_lib_item)
      tryCatch(unarchive_library_item(paths, input$unarchive_lib_item), error = function(...) NULL)
      lib_archive_trigger(lib_archive_trigger() + 1L)
    }, ignoreNULL = TRUE)

    observeEvent(input$remove_lib_item, {
      req(input$remove_lib_item)
      rel_path <- input$remove_lib_item
      showModal(modalDialog(
        title = tagList(icon("triangle-exclamation"), " Remove library item"),
        tagList(
          tags$p(sprintf("Remove '%s' from your Metis library index?", basename(rel_path))),
          tags$p(style = "font-size:0.84rem; color:var(--metis-text-muted);",
                 "The file on disk will NOT be deleted — only the library index record.")
        ),
        footer = tagList(
          modalButton("Cancel"),
          actionButton(ns("confirm_remove_lib"), "Remove from index", class = "btn-danger")
        ),
        easyClose = FALSE
      ))
    }, ignoreNULL = TRUE)

    observeEvent(input$confirm_remove_lib, {
      tryCatch(remove_library_item(paths, input$remove_lib_item), error = function(...) NULL)
      removeModal()
      lib_archive_trigger(lib_archive_trigger() + 1L)
    })

    # Archived item count badge
    output$archived_count <- renderUI({
      lib_archive_trigger()
      statuses <- get_library_item_statuses(paths)
      n_arch <- sum(statuses$status == "archived", na.rm = TRUE)
      if (n_arch > 0L)
        tags$span(sprintf("(%d archived)", n_arch), style = "color:var(--metis-text-muted);")
    })
    ...
    output$literature_view <- renderUI({
      ...
      # Apply archive/remove filter
      statuses <- get_library_item_statuses(paths)
      show_archived <- isTRUE(input$show_archived)

      if (nrow(statuses) && "relative_path" %in% names(statuses)) {
        removed_paths  <- statuses$relative_path[statuses$status == "removed"]
        archived_paths <- statuses$relative_path[statuses$status == "archived"]
        # Always hide removed items
        if (length(removed_paths)) s <- s[!s$relative_path %in% removed_paths, , drop = FALSE]
        # Hide archived unless show_archived ticked
        if (!show_archived && length(archived_paths)) {
          s <- s[!s$relative_path %in% archived_paths, , drop = FALSE]
        }
      }

      if (!nrow(s)) {
        return(div(class="empty-state",
          tagList(icon("box-archive"), " All items are archived or removed."),
          tags$br(),
          tags$small("Tick 'Show archived' above to see them.")
        ))
      }

      if (lit_view() == "gallery") {
        gallery_cards_with_actions(s, ns, statuses, show_archived)
      } else {
        lit_table_with_actions(s, ns, statuses, show_archived)
      }
    })
...
# ── Gallery cards with archive/remove actions ───────────────────────────────

gallery_cards_with_actions <- function(s, ns, statuses, show_archived) {
  n       <- min(nrow(s), 60L)
  rows    <- s[seq_len(n), , drop = FALSE]
  palette <- c("#174c4f","#2d6073","#2e6b4f","#b36a1d","#7b4f2e",
               "#5c3d8f","#8f3d3d","#3d6e4e","#4e5c8f","#8f7b3d")

  archived_paths <- if (nrow(statuses)) statuses$relative_path[statuses$status == "archived"] else character()
  ...
  cards <- lapply(seq_len(nrow(rows)), function(i) {
    ...
    note    <- if ("relevance_note" %in% names(row) && !is.na(row$relevance_note))
                 row$relevance_note else ""
    relpath <- if ("relative_path" %in% names(row)) row$relative_path else ""
    col     <- palette[((i - 1L) %% length(palette)) + 1L]
    is_arch <- relpath %in% archived_paths

    div(class = "gallery-card",
      style = if (is_arch) "opacity:0.55;" else "",
      div(class = "gallery-card-bucket", style = paste0("background:", col, ";"), bucket),
      div(class = "gallery-card-title", fname),
      if (nzchar(mode)) div(class = "gallery-card-mode", mode),
      if (nzchar(note)) div(class = "gallery-card-note", note),
        ...
        if (is_arch) {
          ...
          tagList(
            tags$button(
              ...
              onclick = sprintf("Shiny.setInputValue('%s','%s',{priority:'event'})",
                                ns("archive_lib_item"), relpath),
              "Archive"
            ),
            tags$button(
              class = "btn btn-xs btn-remove",
              onclick = sprintf("Shiny.setInputValue('%s','%s',{priority:'event'})",
                                ns("remove_lib_item"), relpath),
              "Remove"
            )
          )
        }
  ...
  tagList(
    if (nrow(s) > 60L)
      tags$p(style = "font-size:0.78rem; color:var(--metis-text-muted); margin-bottom:0.5rem;",
             sprintf("Showing 60 of %d items.", nrow(s))),
    div(class = "gallery-grid", cards)
  )
}

# ── Table view with archive/remove actions ───────────────────────────────────

lit_table_with_actions <- function(s, ns, statuses, show_archived) {
  archived_paths <- if (nrow(statuses)) statuses$relative_path[statuses$status == "archived"] else character()
  rows <- head(s, 60L)
  ...
  tagList(lapply(seq_len(nrow(rows)), function(i) {
    ...
    div(style = if (is_arch) "opacity:0.55; margin-bottom:0.3rem;" else "margin-bottom:0.3rem;",
      div(style = "display:flex; align-items:center; gap:0.5rem; padding:0.4rem 0; border-bottom:1px solid var(--metis-border);",
        div(style = "flex:1;",
          tags$span(style = "font-weight:500; font-size:0.85rem;", fname),
