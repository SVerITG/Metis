ensure_db_schema <- function(paths) {
  ...
  if (!"launcher_type" %in% project_cols) {
    DBI::dbExecute(con, "ALTER TABLE projects ADD COLUMN launcher_type TEXT")
  }
  if (!"launcher_path" %in% project_cols) {
    DBI::dbExecute(con, "ALTER TABLE projects ADD COLUMN launcher_path TEXT")
  }
  ...
  # Migrate legacy phd_milestones → research_milestones
  old_tables <- DBI::dbGetQuery(con, "SELECT name FROM sqlite_master WHERE type='table' AND name='phd_milestones'")
  if (nrow(old_tables) > 0L) {
    tryCatch({
      DBI::dbExecute(con, "INSERT OR IGNORE INTO research_milestones SELECT * FROM phd_milestones")
      DBI::dbExecute(con, "DROP TABLE phd_milestones")
  ...
  # ── Phase 4 — Journal, Contacts, Intelligence, Brainstorm ────────────────
  DBI::dbExecute(con, "
  ...
  # Self-improvement proposals — agents propose, user approves
  DBI::dbExecute(con, "
  ...
  # Separate archive/remove status for library items — survives TSV refreshes
  DBI::dbExecute(con, "
  ...
  # Seed default user topics if none exist
  topic_count <- DBI::dbGetQuery(con, "SELECT COUNT(*) AS n FROM user_topics")$n[[1]]
  if (identical(topic_count, 0L) || identical(topic_count, 0)) {
    now_ts <- format(Sys.time(), "%Y-%m-%d %H:%M:%S")
    topics <- data.frame(
      topic = c("sleeping sickness", "public health", "AI developments"),
      description = c(
        "HAT, trypanosomiasis, NTDs, elimination surveillance",
        "Epidemiology, surveillance systems, global health",
        "AI tools, LLMs, Claude, agentic systems"
      ),
      active = c(1L, 1L, 1L),
      created_at = rep(now_ts, 3),
      stringsAsFactors = FALSE
    )
    DBI::dbWriteTable(con, "user_topics", topics, append = TRUE)
  }

  # ALTER TABLE agent_runs — add token tracking columns if not present
  ar_cols <- DBI::dbGetQuery(con, "PRAGMA table_info(agent_runs)")$name
  if (!"input_tokens"  %in% ar_cols) DBI::dbExecute(con, "ALTER TABLE agent_runs ADD COLUMN input_tokens  INTEGER DEFAULT 0")
  if (!"output_tokens" %in% ar_cols) DBI::dbExecute(con, "ALTER TABLE agent_runs ADD COLUMN output_tokens INTEGER DEFAULT 0")
  if (!"model"         %in% ar_cols) DBI::dbExecute(con, "ALTER TABLE agent_runs ADD COLUMN model         TEXT DEFAULT ''")
  if (!"session_id"    %in% ar_cols) DBI::dbExecute(con, "ALTER TABLE agent_runs ADD COLUMN session_id    TEXT DEFAULT ''")

  # ── tracked_files: column migrations ─────────────────────────────
  tf_cols <- DBI::dbGetQuery(con, "PRAGMA table_info(tracked_files)")$name
  if (length(tf_cols) && !"label" %in% tf_cols)
    DBI::dbExecute(con, "ALTER TABLE tracked_files ADD COLUMN label TEXT DEFAULT ''")
  if (length(tf_cols) && !"watch" %in% tf_cols)
    DBI::dbExecute(con, "ALTER TABLE tracked_files ADD COLUMN watch INTEGER DEFAULT 0")

  # ── Planner: category column on tasks ────────────────────────────
  task_cols <- DBI::dbGetQuery(con, "PRAGMA table_info(tasks)")$name
  if (!"category" %in% task_cols)
    DBI::dbExecute(con, "ALTER TABLE tasks ADD COLUMN category TEXT DEFAULT 'project'")

  # ── Planner: focus_items table ────────────────────────────────────
  DBI::dbExecute(con, "
  ...
  # ── Phase 4: pipeline session tables ──────────────────────────────
  DBI::dbExecute(con, "
...
insert_daily_insight <- function(paths, content, sources = "") {
  ...
  DBI::dbExecute(
    con,
    "INSERT OR REPLACE INTO daily_insights (insight_date, content, sources, generated_at) VALUES (?, ?, ?, ?)",
    params = list(today, content, sources, format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
  )
  invisible(TRUE)
}

get_daily_insight <- function(paths, date = format(Sys.Date())) {
  tryCatch(
    db_table(paths, sprintf(
      "SELECT * FROM daily_insights WHERE insight_date = '%s' LIMIT 1", date
    )),
    error = function(...) data.frame()
  )
}

insert_publication <- function(paths, title, journal = "", pub_date = "", doi = "",
                               topic_tag = "", relevance_note = "", source_url = "") {
  ensure_db_schema(paths)
  con <- connect_db(paths)
  on.exit(DBI::dbDisconnect(con), add = TRUE)
  DBI::dbExecute(
    ...
    paste(
      ...
      "(title, journal, pub_date, doi, topic_tag, relevance_note, source_url, discovered_at)",
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    ),
    params = list(title, journal, pub_date, doi, topic_tag, relevance_note,
                  source_url, format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
  )
  invisible(TRUE)
}

archive_library_item <- function(paths, relative_path) {
  ensure_db_schema(paths)
  con <- connect_db(paths)
  on.exit(DBI::dbDisconnect(con), add = TRUE)
  DBI::dbExecute(
    con,
    "INSERT OR REPLACE INTO library_item_status (relative_path, status, updated_at) VALUES (?, 'archived', ?)",
    params = list(relative_path, format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
  )
  invisible(TRUE)
}

unarchive_library_item <- function(paths, relative_path) {
  con <- connect_db(paths)
  on.exit(DBI::dbDisconnect(con), add = TRUE)
  DBI::dbExecute(
    con,
    "INSERT OR REPLACE INTO library_item_status (relative_path, status, updated_at) VALUES (?, 'active', ?)",
    params = list(relative_path, format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
  )
  invisible(TRUE)
}

remove_library_item <- function(paths, relative_path) {
  con <- connect_db(paths)
  on.exit(DBI::dbDisconnect(con), add = TRUE)
  DBI::dbExecute(
    con,
    "INSERT OR REPLACE INTO library_item_status (relative_path, status, updated_at) VALUES (?, 'removed', ?)",
    params = list(relative_path, format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
  )
  invisible(TRUE)
}

get_library_item_statuses <- function(paths) {
  tryCatch(
    db_table(paths, "SELECT relative_path, status FROM library_item_status"),
    error = function(...) data.frame(relative_path = character(), status = character())
  )
}

get_publications <- function(paths, unread_only = TRUE, limit = 20L) {
  tryCatch({
    where <- if (unread_only) "WHERE (read_at IS NULL OR read_at = '')" else ""
    db_table(paths, sprintf(
      "SELECT * FROM new_publications %s ORDER BY discovered_at DESC LIMIT %d",
      where, as.integer(limit)
    ))
  }, error = function(...) data.frame())
}

mark_publication_read <- function(paths, pub_id) {
  con <- connect_db(paths)
  on.exit(DBI::dbDisconnect(con), add = TRUE)
  DBI::dbExecute(
    con,
    "UPDATE new_publications SET read_at = ? WHERE id = ?",
    params = list(format(Sys.time(), "%Y-%m-%d %H:%M:%S"), as.integer(pub_id))
  )
  invisible(TRUE)
}

get_user_topics <- function(paths) {
  tryCatch(
    db_table(paths, "SELECT * FROM user_topics WHERE active = 1 ORDER BY topic"),
    error = function(...) data.frame()
  )
}

log_agent_run <- function(paths, agent_slug, task_summary, input_path = "",
                          output_path = "", status = "completed",
                          input_tokens = 0L, output_tokens = 0L, model = "") {
  ensure_db_schema(paths)
  con <- connect_db(paths)
  on.exit(DBI::dbDisconnect(con), add = TRUE)
  DBI::dbExecute(
    ...
    params = list(agent_slug, task_summary, input_path, output_path, status,
                  as.integer(input_tokens), as.integer(output_tokens), model,
...
insert_research_milestone <- function(paths, article_title, target_date, status, notes) {
  ensure_db_schema(paths)
  con <- connect_db(paths)
  on.exit(DBI::dbDisconnect(con), add = TRUE)
...
get_research_milestones <- function(paths) {
  db_table(paths, "SELECT * FROM research_milestones ORDER BY target_date ASC")
}
...
seed_default_data <- function(paths) {
  ...
  if (identical(project_count, 0L) || identical(project_count, 0)) {
    ...
    projects <- data.frame(
      project_id = c("phd-framework", "passive-screening-drc", "metis-dashboard", "personal"),
      title = c("PhD framework", "Passive screening in DRC", "Metis dashboard", "Personal"),
      domain = c("phd", "sleeping sickness", "software", "personal"),
      ...
      created_at = rep(now, 4),
      stringsAsFactors = FALSE
    )
    DBI::dbWriteTable(con, "projects", projects, append = TRUE)
  } else {
    # Ensure Personal project exists for existing installs
    personal_count <- DBI::dbGetQuery(con, "SELECT COUNT(*) AS n FROM projects WHERE project_id = 'personal'")$n[[1]]
    if (identical(personal_count, 0L) || identical(personal_count, 0)) {
      now <- format(Sys.time(), "%Y-%m-%d %H:%M:%S")
      DBI::dbExecute(con,
        "INSERT OR IGNORE INTO projects (project_id, title, domain, status, priority, next_step, created_at) VALUES (?,?,?,?,?,?,?)",
        params = list("personal", "Personal", "personal", "active", "medium", "Add personal tasks", now)
      )
    }
  }
...
insert_project <- function(paths, title, domain, status, priority, next_step,
  ...
  DBI::dbExecute(
    ...
    paste(
      ...
      "(project_id, title, domain, status, priority, next_step, launcher_type, launcher_path, created_at)",
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
    ),
    params = list(project_id, title, domain, status, priority, next_step,
                  launcher_type %||% "", launcher_path %||% "",
                  format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
  )
...
# ── Planner: focus_items helpers ──────────────────────────────────────────

get_focus_items <- function(paths, week) {
  tryCatch(
    db_table(paths, sprintf(
      "SELECT item_id, item_type, week, COALESCE(label,'') AS label, pinned_at
       FROM focus_items WHERE week = '%s' ORDER BY pinned_at ASC",
      week
    )),
    error = function(...) data.frame(
      item_id = character(), item_type = character(),
      week = character(), label = character(), pinned_at = character()
    )
  )
}

pin_focus_item <- function(paths, item_id, item_type, label, week) {
  ensure_db_schema(paths)
  con <- connect_db(paths)
  on.exit(DBI::dbDisconnect(con), add = TRUE)
  DBI::dbExecute(con,
    "INSERT OR REPLACE INTO focus_items (item_id, item_type, week, label, pinned_at) VALUES (?,?,?,?,?)",
    params = list(item_id, item_type, week, label, format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
  )
  invisible(TRUE)
}

unpin_focus_item <- function(paths, item_id, week) {
  con <- connect_db(paths)
  on.exit(DBI::dbDisconnect(con), add = TRUE)
  DBI::dbExecute(con,
    "DELETE FROM focus_items WHERE item_id = ? AND week = ?",
    params = list(item_id, week)
  )
  invisible(TRUE)
}

get_all_plannable <- function(paths) {
  tryCatch({
    tasks <- db_table(paths,
      "SELECT t.task_id AS item_id, 'task' AS item_type,
              COALESCE(p.title || ' / ', '') || t.title AS label
       FROM tasks t
    ...
    projects <- db_table(paths,
      "SELECT project_id AS item_id, 'project' AS item_type, title AS label
       FROM projects WHERE status = 'active' ORDER BY title"
    )
    rbind(tasks, projects)
  }, error = function(...) data.frame(
    item_id = character(), item_type = character(), label = character()
  ))
}
