metis_paths <- function(app_root) {
  ...
  list(
    ...
    research_root = file.path(second_brain_root, "03_domains", "research"),
...
control_room_metrics <- function(paths) {
  ...
  list(
    ...
    research_documents = safe_count_files(paths$research_root)
  )
}
...
git_project_status <- function(local_path) {
  ...
  git_ok <- function(...) {
    tryCatch(
      suppressWarnings(system2("git", c("-C", path, ...), stdout = TRUE, stderr = FALSE)),
      error = function(...) NULL
    )
  }
...
random_research_paper <- function(paths) {
  seeded <- safe_read_tsv(file.path(paths$literature_metadata, "library-phd-seeded.tsv"))
...
search_all_sources <- function(paths, query, max_per = 10L) {
  ...
  search_dirs <- c(paths$research_root, paths$meetings_root)
  md_hits     <- character()
