...
# Verify we landed in the right place (should end in metis)
if (!grepl("metis", basename(second_brain), ignore.case = TRUE)) {
  cat(sprintf("   WARNING: Expected metis, found: %s\n", basename(second_brain)))
...
if (ok) {
  ...
  cat("  Option C (startup):  copy launch_metis_background.vbs shortcut to shell:startup\n\n")
