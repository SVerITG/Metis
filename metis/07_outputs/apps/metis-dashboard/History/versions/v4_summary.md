...
- The agent's project-specific context document (from `02_agents/<agent-slug>/<project>-context.md`, falling back to the agent README)
...
- Left sidebar: all agents discovered by scanning the `02_agents/` filesystem directory
- Main panel: on agent selection — open tasks assigned to that agent, latest output files from `07_outputs/reviews/<agent>/`, mentions in triage logs, and the agent's own documents
...
- Agent Output page depends on the existence of `07_outputs/reviews/<agent>/` directories — if absent, the panel renders empty without error (graceful, but potentially confusing)
...
3. Create `07_outputs/reviews/<agent-slug>/` directories for agents that will produce file outputs
4. Ensure `02_agents/<agent-slug>/` directories exist for all agents listed in `app.R`
