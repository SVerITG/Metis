...
| **PII detection** | 14 checks, 4-level classification. Hard block on sensitive data reaching the API |
| **Injection probe** | Detects prompt injection in external content (papers, transcripts) |
| **Constitution** | 12 machine-readable rules applied to every deep agent run |
...
3. `safety.py` — 14 PII checks, 4-level classification, hard block on sensitive data reaching API
4. `constitution.md` — 12 machine-readable rules for deep and chained agent runs
...
| `system/config/constitution.md` | 12 rules applied to every deep/chain run |
