...
| **9-tab dashboard** | Today · News · Knowledge · Meetings · Learning · Work · Reflection · Presentation · Metis — all live, all local |
...
| **Reflection** | Three piles that are actually different — ideas, a journal, and threads. A thread is a question you keep returning to; open one and the graph shows that thread and what it touches, one hop out. Plus an archive, because nothing here is deleted. |
| **Presentation** | An index of the slide decks you have already delivered, grouped by collection — and the lesson lineage on top: which talks you have given more than once, where, and what you changed each time. Master slides become named styles; the builder assembles a brief rather than slides. |
...
| **The zone pattern, and the surfaces rebuilt on it** — a box now says what it holds with a coloured bar and, where there is more than one view of the same material, a tab strip instead of a stack. Three surfaces were rebuilt on it. A subject shelf that stacked eleven panels down one page became five tabs, and finally grew the panel it had always promised: what is closest to *your own work*, ranked against your projects and courses rather than against the subject — top N, never a floor, with the denominator printed above the list. |
| **The reflection surface was flat for a reason no layout could fix** — the thread rail and the centre panel both selected from the same table, so "thread" and "idea" were two renderings of one thing and could not mean anything different however they were labelled. A thread is now a distinct kind, with the sources it touches, and a graph scoped to it. Drawing the whole graph is what made the old one unreadable: a graph of everything is a picture of nothing. |
| **Eight hundred presentations the dashboard did not know existed** — the folders can say where a file is; they cannot say what you have already presented about this, to whom, and how you said it differently last time. A read-only index answers that, joining the same lesson across the collections it was delivered in. Author, audience and language register are written by hand because no file carries them — and the scanner names the columns it owns, so re-indexing cannot erase them. |
| **An index that deleted itself on a short walk** — re-indexing removes what is no longer on disk, which is right until the walk is incomplete. An interrupted scan, an unmounted drive and a caller passing a subset all look identical to "everything was deleted", and the pruning is instant and total. A short walk is now refused and reported; a control confirms a genuine small deletion still applies. |
| **A hidden tab never loaded its own contents** — a panel deferred until first shown used a trigger that fires when an element scrolls into view, and an element hidden with the `hidden` attribute has no box to scroll into. Un-hiding it is not a reveal, so the panel would have stayed empty forever and read as a server error. |
| **The test harness was looking for the database inside the repository** — a path retired mid-year because a synced folder eats the write-ahead sidecars. It had been reporting a healthy database as missing on every run since. Two other checks asserted an architecture the dashboard no longer uses and failed on every tab for it. Critical failures went from thirteen to zero; the checks now assert behaviour instead of shape. |
