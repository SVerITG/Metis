...
```
get_agent_context("meeting-memory")
```
...
This indirection is deliberate: your real prompt lives in `agents/meeting-memory/` and is
...
1. `log_agent_run(agent_slug="meeting-memory", task_summary="<one line>")`
2. If the researcher stated or confirmed a standing preference — how something
   ...
   next time:
   `record_decision(decision="...", category="...", agent_slug="meeting-memory", context="why")`
