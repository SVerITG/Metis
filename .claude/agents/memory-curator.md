...
```
get_agent_context("memory-curator")
```
...
This indirection is deliberate: your real prompt lives in `agents/memory-curator/` and is
...
1. `log_agent_run(agent_slug="memory-curator", task_summary="<one line>")`
2. If the researcher stated or confirmed a standing preference — how something
   ...
   next time:
   `record_decision(decision="...", category="...", agent_slug="memory-curator", context="why")`
