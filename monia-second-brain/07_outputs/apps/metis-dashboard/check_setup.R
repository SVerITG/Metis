...
required <- c(
  ...
  "xml2", "here", "taskscheduleR", "visNetwork",
  "jsonlite"
)
...
# Verify we landed in the right place (should end in monia-second-brain)
if (!grepl("monia-second-brain", basename(second_brain), ignore.case = TRUE)) {
  cat(sprintf("   WARNING: Expected monia-second-brain, found: %s\n", basename(second_brain)))
...
if (ok) {
  ...
  cat("  Option B (browser):  double-click launch_monia.bat\n")
  cat("  Option C (startup):  copy launch_monia_background.vbs shortcut to shell:startup\n\n")
  cat("First run: source('inst/scripts/seed_projects.R') to populate projects.\n")
} else {
