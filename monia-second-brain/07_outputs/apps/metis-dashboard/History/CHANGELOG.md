...
Monia promoted to universal router and single default entry point. Routing Protocol, complexity model, and Recording Protocol encoded in her system prompt and contract. Agent run tracking added to the data layer. Seventeen output directories created. CLAUDE.md rewritten around the `/monia` default.

### Added
- **Monia Routing Protocol** (02_agents/metis/system-prompt.md): 5-step protocol — analyze request, select agent(s), assess complexity, announce plan, execute and record; Monia now announces routing decision before executing
- **Routing table** (02_agents/metis/system-prompt.md): full input-type to primary/secondary agent mapping; covers all 18 agents with chaining rules
- **Complexity model** (02_agents/metis/system-prompt.md, CLAUDE.md): four levels — quick (haiku), standard (sonnet), deep (opus), chain (opus + subagents); complexity assessed by Monia, not specified by user
- **Recording Protocol** (02_agents/metis/system-prompt.md): output file template (`07_outputs/reviews/{agent-slug}/{YYYY-MM-DD}_{task-slug}.md`), DB logging rules, mandate to never skip recording for reviews/analyses/searches
...
- **`log_agent_run()` function** (R/data_store.R): CRUD helper; called by Monia after every substantive run to create the DB record
- **17 output directories** (07_outputs/reviews/): one subdirectory per agent slug, pre-created and ready to receive output files
- **Monia quick-invoke templates expanded** (R/mod_agents.R): six templates — general request, morning briefing, full paper review chain, triage, weekly review, "what am I overlooking?"

### Changed
- **Monia contract** (02_agents/metis/contract.md): rewrite — Monia now explicitly owns routing decisions, complexity assessment, and PKM recording; complexity table added; recording mandate ("Never skip recording for reviews, analyses, or searches")
- **CLAUDE.md**: `/monia` documented as default entry point; multi-agent routing example added; complete workflow diagram (Dashboard → Claude Code → Agent → Output → Dashboard); Option A (Monia routes) / Option B (direct call) replaces old invocation section; complexity levels table added
...
### Added
- **Agent: news-aggregator** (02_agents/news-aggregator/): automated RSS feed collection and curation; files: system-prompt.md, contract.md, README.md; security tier: internet access granted (unique among agents)
- **Agent: ux-engineer** (02_agents/ux-engineer/): design system and UI/UX specialist; hard rules encoded in system prompt: no emoji icons, WCAG AA compliance, responsive breakpoints; files: system-prompt.md, contract.md, README.md
- **Agent: epidemiologist** (02_agents/epidemiologist/): senior specialist reviewer with Socratic questioning persona; HAT/NTD expertise; files: system-prompt.md, contract.md, README.md
...
- **02_agents/metis/security-policy.md**: 3 new rows added to per-agent security table — News Aggregator (internet access granted), UX Engineer (no internet), Epidemiologist (no internet)
...
- **Agent: learning-coach** (02_agents/learning-coach/): skill progression, learning paths, statistics focus; files: system-prompt.md, contract.md, README.md
- **Agent: career-coach** (02_agents/career-coach/): EU job prep, CV support, career strategy; files: system-prompt.md, contract.md, README.md
- **Agent: personal-finance** (02_agents/personal-finance/): market awareness, financial trends; files: system-prompt.md, contract.md, README.md
...
- **02_agents/CLAUDE.md**: three new agent invocations added — /learning-coach, /career-coach, /personal-finance; agent count: 11 -> 14
...
### Added
- **Agent coverage audit** (02_agents/): all 11 agents audited against all 6 active projects; 7 coverage gaps identified and filled
- **6 new agent context documents** (02_agents/): hat-dashboard-context.md (dashboard-engineer), hat-clustering-context.md (phd-architect), hat-clustering-context.md (methods-coach), multilevel-analysis-course-context.md (methods-coach), hat-clustering-context.md (writing-partner), multilevel-analysis-course-context.md (presentation-maker) — see ADR-012
...
- **Security policy** (02_agents/metis/security-policy.md): system-wide data protection policy with data classification table (SENSITIVE/CONFIDENTIAL/INTERNAL/PUBLIC), local-first principle, code security rules (SQL, filesystem, external commands, secrets, Shiny UI), privacy rules for research data (HAT case records = SENSITIVE), per-agent security table, and escalation rules; derived from Ruflo v3.5 security module

### Changed
- **02_agents/dashboard-engineer/system-prompt.md**: Shiny UI security section added
- **02_agents/meeting-memory/system-prompt.md**: local-first privacy rules added
- **02_agents/librarian/system-prompt.md**: data sourcing protection rules added
- **02_agents/news-radar/system-prompt.md**: data hygiene and scope rules added
- Note: 02_agents/software-engineer/system-prompt.md was NOT modified — already has a comprehensive security checklist
...
- **Agent Invocation from Task Cards** (R/mod_projects.R): kanban cards with an agent owner show an [Invoke] button
  ...
  - Agent context lookup: `02_agents/<slug>/<project-context>.md` with fallback to `README.md`
...
- **Agent Output Page** (R/mod_agents.R): NEW module, registered as "Agents" tab under `nav_menu("More")`
  - Left sidebar: lists all agents discovered from the `02_agents/` filesystem
  - Main panel: open tasks assigned to that agent, latest output files from `07_outputs/reviews/<agent>/`, triage log mentions, agent documents
...
- **Agent Context Documents** (02_agents/): two new project-specific context files
  - `02_agents/software-engineer/hat-dashboard-context.md` — HAT Dashboard RAG context (branch `server`, 14 modified files)
  - `02_agents/software-engineer/mlm-course-context.md` — MLM Course Node.js app, Quarto files, spatial scan module
...
First working shell of the Monia Dashboard. All pages scaffolded, SQLite backend established, library scan and meeting import wired, news briefs stored, projects and tasks in database.
...
- Launcher: launch_monia.bat (double-click) and launch_monia_background.vbs (silent startup)
