...
| # | Item | Why / evidence | Files | Size·Impact |
|---|---|---|---|---|
| 2.1 | **A single Settings surface** — ⚠️ **and it got WORSE today, measurably.** The Metis Systems surface now carries **20 top-level sections**, two of them added this session (Where settings live, Update). Roughly seven are settings-ish (How Metis talks to you · Default model · Theme · Feature tips · Startup & persistence · Where settings live · Update) sitting as separate stacked sections rather than one pane. Phase S consolidated *scattered controls into sections*; the remaining work is consolidating *those sections into one Settings area* — otherwise each genuinely useful addition makes the page longer, which is the sprawl 2.1 exists to fix. Deliberate UI restructure, worth doing with the owner's eye on it rather than blind. | Settings are spread across the Metis tab (12 sections), the wizard (13 sections), ~40 config files, and a chat layer. Consolidate into one coherent, searchable pane; keep chat + file access for power users. | `routers/metis_tab.py`, `setup.py`, `templates/metis_tab.html` | L·★★ |
...
| 2.3 | ~~**Generalize the update-menu declutter pattern**~~ **✅ SUBSTANTIALLY MET — verified 2026-08-12, no new work needed.** The `_update_menu` macro is already invoked at 5 real call sites (global header, Today board boxes, literature discovery, morning brief, news rail) with honest "no API tokens" vs "uses credit" labels. Control counts were then measured per surface: the busiest template has 14, routers add at most 4 — there is no crowded cluster left that a popover would improve. Applying it further would be decluttering nobody needs. | Reuse `partials/_update_menu.html` to collapse crowded control clusters across surfaces into one action + on-demand popover with honest labels. Inventory every button first. | `partials/_update_menu.html`; all surface templates | M·★★ |
...
> **Status: ✅ COMPLETE (2026-08-12)** — `9614220` (5.1) · `9b1f22a` (5.2) · `9ea6058` (5.3) ·
> `b8a1695` (5.4). All four verified against the running system.
> - **5.1** `/api/v1` — health, projects, tasks, library, search (cited passages), capture. Carries
>   its OWN bearer token, because the dashboard's Origin check is necessary and not sufficient: an
>   add-in has its own origin, and a GET gets no Origin check at all while these endpoints read the
>   researcher's library. Token generated, not configured. No tool-execution endpoint, deliberately.
> - **5.2** PowerPoint read-back + ingestion with provenance, and generation honours
>   `system/config/templates/deck.pptx`. **Spreadsheets are treated differently on purpose**: structure
>   only, never cell values, and ingest refused — dumping cells would have driven a coach and horses
>   through "send code, not data" via a file-reading convenience nobody would audit.
> - **5.3** `job_office_sync` (hourly) re-reads decks changed on disk, so an edit in PowerPoint flows
>   back unprompted. Required implementing `every_hours` in the scheduler: an unrecognised schedule key
>   was SILENTLY IGNORED and would have registered the job at the default 07:00, looking scheduled.
> - **5.4** Taskpane + manifest + the HTTPS bridge that makes it possible at all (self-signed cert for
>   `localhost` AND 127.0.0.1, optional `METIS_HTTPS=1` on a separate port, CORS scoped to Office hosts
>   and to `/api/v1` only). **HONEST LIMIT: not run inside Office** — that needs Windows with
>   PowerPoint. Sideload steps documented as untested in `system/office-addin/README.md`.
>
> **Phase 5 acceptance, against the original wording:** a deck generated by Metis uses the researcher's
> template ✅; editing it in PowerPoint updates Metis ✅ (hourly, unprompted); every ingested artifact
> carries provenance back to its source path ✅.
