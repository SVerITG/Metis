...
## Latest status — 2026-09-04 (session 6) — read this first

**84 commits since the last status block.** The plan's phases are done; what has been
running since is a surface-by-surface repair pass, and its findings all have one shape.

**Theme: the silent failure.** Session 5 found writes with no reader. This pass found
*features that reported success while doing nothing*, which is the same defect wearing
a working UI. In one fortnight: dashboard **semantic search had never worked** (the
document embedding prefix was used for queries, with no normalisation); **two of the
three Work views could never be shown** (`u-hide` carries `!important`); **five panels
reported numbers they had never measured**; **every write in the system failed for five
minutes after each restart** (the boot news-scan held one write transaction across every
feed fetch); **2,282 papers were never triaged and could not be reached**; **763 library
rows carried a DOI belonging to a neighbouring feed item**; 3,143 duplicate corpus chunks;
and **four rival counts of one backlog**, one of which drew 34 cancelled tasks as live work.
None of these announced themselves. Every one was found by measuring the feature rather
than reading the code.

**Shipped since 2026-08-17 (38 feat · 22 fix):** Work surface phases 2–5 (categories you
own, a board where dragging *is* the edit, dated tasks on the calendar, launchers that
reflect real capability — the root cause there was a missing write path, `external_path`
was never editable after creation, so 7 projects were folderless forever) · Today surface
rebuilt (one box for what is new, pins that follow a subject, recall gated on lessons
actually read) · Focus areas as user-owned surfaces with per-shelf sections · News and
briefings with a real per-article description and image · Learning: a methodology library,
a reading course that stops setting homework, a course reader that scrolls and links ·
one search dialog replacing two boxes that disagreed · identity removed from what an
earlier session had published to two PUBLIC repos.

**Closed this session:** the focus lens matched short keywords as stems, so `ai` was never
in the AI lens at all and the surface under-reported its own subject by roughly half
(23 of 4,103 briefs returned, 21 more silently dropped). Fixed, with 17 tests pinning
both the catch and the refusal. Two tools added: `seed_focus_content.py` (authored surface
content, local overriding shipped, rejecting a section key the template cannot render)
and `audit_publication_dois.py` (rows whose DOI resolves to a *different* paper).

**Also closed: the base shell had two escape routes.** `origin` is the public
domain-agnostic repo and its `main` is generated by `build-base-shell.sh`, which strips the
domain courses. But the strip only runs on that one path — **a pushed feature branch walks
...
still have manifests with no URLs · the ambient matcher drops 3-letter tokens, so
short domain acronyms never contribute to a procedure match · `remind_at` is stored and nothing
acts on it, so reminders never fire · no time-of-day or duration on a plan, which blocks
lead-time alerts.

**Still open, owner decisions:** `knowledge_links` / `learning_competencies` /
`research_milestones` read and never written · an external analysis pipeline's output map (~280
files across 20+ `Results/` folders, nothing recording which run produced which), plus two
unresolved questions in it — a disputed cluster count, and whether a merged script
supersedes the three it was merged from · whether
to rewrite git history to remove personal detail from older commit *messages* (measured,
never run; needs `git-filter-repo` installed).

**Immediate, owner:** confirm the Anthropic API key was rotated. `system/.env` still holds
one and was last modified 2026-08-21, *after* C10 was raised — so it may already be done
and the plan simply not updated. Cannot be verified from outside without printing the key.

**Backlog:** 84 open / 48 done. Most open items are follow-ups the shipped work itself
generated, not forgotten promises.

---

## Status — 2026-08-17 (session 5)
...
**Corollary added 2026-09-04 — a guard on the ceremonial path is a convention.**

> **When a protection runs on one route to an outcome, enumerate the other routes.**
> The everyday route is the one people actually take, and it is usually the unguarded one.

`build-base-shell.sh` strips domain content when generating the public base repo from
`main`. That is the ceremonial path. `git push origin <branch>` reaches the same public
repo and runs no strip at all — and it is the path anyone reaches for casually. Two
branches had taken it, one of them four months earlier. The script was not wrong; it was
simply not on the path being used, which is the same shape as a context file that
correctly describes a loading mechanism nothing invokes.
