...
|-----------|----------|---------|
| MCP Server (Python) | `system/mcp-server/` (OneDrive) | WSL (Linux) |
| MCP venv | `~/.local/share/metis-mcp/.venv` (local) | WSL |
| Shiny Dashboard (R) | `system/app-py/` (OneDrive) | Windows RStudio |
| SQLite database | `system/app/data/metis.sqlite` | Shared via OneDrive |
...
```bash
cd "/mnt/c/Users/<user>/OneDrive/Documents/7. Software/PKM/metis/system/mcp-server"
bash setup.sh
```
...
```json
{
  "mcpServers": {
    "metis-pkm": {
      "command": "/home/<user>/.local/share/metis-mcp/.venv/bin/metis-mcp",
      "env": {
  ...
  "mcpServers": {
    "metis-pkm": {
      ...
      "args": [
        "-e",
        "bash", "-c",
        "METIS_PKM_ROOT='/mnt/c/Users/<user>/OneDrive/Documents/7. Software/PKM/metis' /home/<user>/.local/share/metis-mcp/.venv/bin/metis-mcp"
      ]
    }
  }
}
...
The wrapper script `/home/<user>/.local/share/metis-mcp/run.sh` sets `METIS_PKM_ROOT`
...
1. Open RStudio
2. Open the project: `system/app-py/`
