...
# ── Quick update mode ────────────────────────────────────────────────────────
# Re-syncs source and reinstalls the package without running the full wizard.
# Usage:  bash setup-mcp.sh --update
#         bash setup-mcp.sh update
_ARG1="${1:-}"
if [ "$_ARG1" = "--update" ] || [ "$_ARG1" = "update" ]; then
  echo ""
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo "  Metis — quick update"
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo ""

  if [ ! -d "$VENV_DIR" ]; then
    echo "ERROR: No existing venv at $VENV_DIR — run a full install first."
    exit 1
  fi

  # Re-copy source
  echo "Copying source to local filesystem..."
  rm -rf "$LOCAL_SRC"
  mkdir -p "$LOCAL_SRC"
  ...
  # Update marker file
  MARKER_FILE="$HOME/.local/share/metis-mcp/.metis-rc-root"
  echo "$METIS_RC_ROOT" > "$MARKER_FILE"

  # Apply migrations
  echo "Applying schema migrations..."
  METIS_RC_ROOT="$METIS_RC_ROOT" "$VENV_DIR/bin/python3" -m metis_mcp.migrations 2>&1 | sed 's/^/  /' || true

  # Verify
  if "$VENV_DIR/bin/python3" -c "import metis_mcp.server; print('  import OK')" 2>&1; then
    echo ""
    echo "  ✓ Metis updated successfully."
    echo "  Restart Claude Code or Claude Desktop to pick up changes."
  else
    echo ""
    echo "  ✗ Import check failed — see above."
    exit 1
  fi
  exit 0
fi
...
# ── Write run.sh ─────────────────────────────────────────────────────────────
# run.sh derives METIS_RC_ROOT at runtime from a .metis-rc-root marker file
# instead of baking an absolute path. This means moving or renaming the repo
# only requires updating the marker (or re-running setup-mcp.sh), not editing
# run.sh. The METIS_RC_ROOT env var still takes priority if set externally.
MARKER_FILE="$HOME/.local/share/metis-mcp/.metis-rc-root"
echo "$METIS_RC_ROOT" > "$MARKER_FILE"
echo "  RC root marker written to $MARKER_FILE"

cat > "$RUN_SCRIPT" <<'RUNSH'
#!/bin/bash
# Auto-generated by setup-mcp.sh.
# METIS_RC_ROOT is resolved at runtime — not baked in at install time.
# Priority: 1) METIS_RC_ROOT env var  2) .metis-rc-root marker file
VENV_DIR="$HOME/.local/share/metis-mcp/.venv"
MARKER="$HOME/.local/share/metis-mcp/.metis-rc-root"

if [ -z "${METIS_RC_ROOT:-}" ]; then
    if [ -f "$MARKER" ]; then
        METIS_RC_ROOT="$(cat "$MARKER" | tr -d '\r\n')"
    else
        echo "ERROR: METIS_RC_ROOT not set and no marker file at $MARKER" >&2
        echo "Fix: re-run setup-mcp.sh or set METIS_RC_ROOT manually." >&2
        exit 1
    fi
fi

if [ ! -d "$METIS_RC_ROOT" ]; then
    echo "ERROR: METIS_RC_ROOT points to a missing directory:" >&2
    echo "  $METIS_RC_ROOT" >&2
    echo "Did you move or rename the Metis folder? Fix:" >&2
    echo "  1. Update $MARKER with the new path, or" >&2
    echo "  2. Re-run: bash <new-path>/system/mcp-server/setup-mcp.sh" >&2
    exit 1
fi

export METIS_RC_ROOT
...
ENV_FILE="$METIS_RC_ROOT/system/.env"
ENV_EXAMPLE="$METIS_RC_ROOT/system/.env.example"
_HAS_KEY=0
if [ -f "$ENV_FILE" ]; then
  grep -q '^ANTHROPIC_API_KEY=sk-ant' "$ENV_FILE" 2>/dev/null && _HAS_KEY=1
fi
[ -n "${ANTHROPIC_API_KEY:-}" ] && case "$ANTHROPIC_API_KEY" in sk-ant-*) _HAS_KEY=1 ;; esac

if [ "$_HAS_KEY" = "0" ] && [ -t 0 ]; then
  echo ""
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo "  Anthropic API key (optional)"
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo ""
  echo "  An API key enables automated features that run while you're away:"
  echo "  · Scheduled morning briefs"
  echo "  · Automated news scans"
  echo "  · Dashboard AI features"
  echo ""
  echo "  You do NOT need this for using Metis through Claude Desktop/Code."
  echo "  Get a key at: https://console.anthropic.com → API Keys → Create Key"
  echo ""
  read -rp "  Paste your API key (sk-ant-...) or press Enter to skip: " _INPUT_KEY
  _INPUT_KEY=$(echo "$_INPUT_KEY" | tr -d '\r\n ')
    ...
    case "$_INPUT_KEY" in
      sk-ant-*)
        ...
        _HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 \
          -H "x-api-key: $_INPUT_KEY" \
          -H "anthropic-version: 2023-06-01" \
          "https://api.anthropic.com/v1/models" 2>/dev/null || echo "000")
        if [ "$_HTTP_CODE" = "200" ]; then
          echo "  ✓ API key is valid"
        elif [ "$_HTTP_CODE" = "401" ]; then
          echo "  ✗ Key rejected by Anthropic (HTTP 401) — saving anyway, check console.anthropic.com"
        elif [ "$_HTTP_CODE" = "000" ]; then
          echo "  · Could not reach API (network/proxy) — saving key, will verify on first use"
        else
          echo "  · API returned HTTP $_HTTP_CODE — saving key, will verify on first use"
        fi
        # Write to .env (create from template if needed)
        if [ ! -f "$ENV_FILE" ] && [ -f "$ENV_EXAMPLE" ]; then
          cp "$ENV_EXAMPLE" "$ENV_FILE"
        fi
        if [ -f "$ENV_FILE" ]; then
          # Replace existing placeholder or empty key
          if grep -q '^ANTHROPIC_API_KEY=' "$ENV_FILE" 2>/dev/null; then
            sed -i "s|^ANTHROPIC_API_KEY=.*|ANTHROPIC_API_KEY=$_INPUT_KEY|" "$ENV_FILE"
          else
            echo "ANTHROPIC_API_KEY=$_INPUT_KEY" >> "$ENV_FILE"
          fi
        else
          echo "ANTHROPIC_API_KEY=$_INPUT_KEY" > "$ENV_FILE"
        fi
        echo "  ✓ Key saved to system/.env"
        export ANTHROPIC_API_KEY="$_INPUT_KEY"
        ;;
      *)
        echo "  · That doesn't look like an Anthropic key (should start with sk-ant-). Skipping."
        echo "  · You can add it later: edit system/.env or use the dashboard settings."
        ;;
    esac
  else
    echo "  · Skipped. You can add a key later in system/.env"
  fi
  unset _INPUT_KEY _HTTP_CODE
fi
unset _HAS_KEY
