...
|------|-----------|--------|----------|
| Windows `.exe` installer | One-click install, no Python knowledge | 🟡 | `metis-setup.iss` complete; Inno Setup compile pending (requires the researcher on Windows) |
...
| Clean-VM test | Verify `.exe` on fresh machine | ⚠️ | Requires the researcher — all code is done |
...
|-----|------------------|--------|
| Windows `.exe` compile + clean-VM test | Personas 1, 2, 4, 6, 7, 9 | the researcher-side action |
