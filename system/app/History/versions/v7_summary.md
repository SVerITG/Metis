...
Two foundational infrastructure pieces are added: an `agent_runs` database table and a `log_agent_run()` function make every agent output traceable from the dashboard. Seventeen output directories under `outputs/reviews/` create a persistent, organized filesystem for all agent work products.
...
- **Recording Protocol**: Output file template (`outputs/reviews/{agent-slug}/{YYYY-MM-DD}_{task-slug}.md`), DB logging rules, and a "never skip recording" mandate for reviews, analyses, and searches.
...
- **17 output directories**: `outputs/reviews/{agent-slug}/` — one per agent, pre-created and ready to receive output files.
...
| outputs/reviews/ | 17 subdirectories created (one per agent slug) |
...
- The Agents tab in the dashboard reads output files from `outputs/reviews/` — it will correctly surface files as soon as they appear, with no dashboard changes required.
