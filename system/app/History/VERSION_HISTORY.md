# Metis Dashboard — Version History
...
Metis becomes the single explicit entry point for the entire agent ecosystem. A formal five-step Routing Protocol, a routing table covering all 18 agents, a four-level complexity model, and a Recording Protocol are encoded in her system prompt and contract. CLAUDE.md is rewritten around the `/metis` default — users no longer need to know the agent taxonomy. An `agent_runs` SQLite table and `log_agent_run()` CRUD function make every agent output traceable and surfaceable in the dashboard. Seventeen output directories under `outputs/reviews/` organize the agent filesystem. Agent count and module count unchanged.

### Headline changes
- Metis Routing Protocol: analyze → select → assess complexity → announce → execute and record
...
- 17 output directories pre-created under `outputs/reviews/`
- CLAUDE.md: Option A (Metis routes) / Option B (direct call); full workflow diagram
- Metis quick-invoke templates: 6 templates covering the primary daily use cases

### New files
| File | Purpose |
|------|---------|
| outputs/reviews/{17 agent dirs} | Output directories for all agent work products |

### Files changed
| File | Change |
|------|--------|
| agents/metis/system-prompt.md | Full rewrite — Routing Protocol, routing table, complexity model, Recording Protocol |
| agents/metis/contract.md | Rewrite — routing ownership, complexity table, recording mandate |
| CLAUDE.md | Default entry point, Option A/B, workflow diagram, complexity levels table |
| R/mod_agents.R | 6 Metis quick-invoke templates |
...
| File | Purpose |
|------|---------|
| agents/news-aggregator/system-prompt.md | RSS aggregator agent — internet-enabled |
| agents/news-aggregator/contract.md | Agent contract |
| agents/news-aggregator/README.md | Agent README |
| agents/ux-engineer/system-prompt.md | UX/design system agent |
| agents/ux-engineer/contract.md | Agent contract |
| agents/ux-engineer/README.md | Agent README |
| agents/epidemiologist/system-prompt.md | Senior epi reviewer agent with Socratic persona |
| agents/epidemiologist/contract.md | Agent contract |
| agents/epidemiologist/README.md | Agent README |
...
| agents/metis/security-policy.md | 3 rows added to per-agent security table |
...
| agents/learning-coach/system-prompt.md | Learning coach agent — skill progression and statistics |
| agents/career-coach/system-prompt.md | Career coach agent — EU job prep and CV support |
| agents/personal-finance/system-prompt.md | Personal finance agent — market awareness |
...
- **6 new context documents** written and placed in the correct `agents/<slug>/` locations — ensures the [Invoke] modal (introduced in v4.0 via ADR-009) always surfaces useful context for each agent/project pair
- **9 new tasks** added to metis.sqlite — all 11 agents now have at least 1 active task visible in the Kanban view
- **Security policy created** at `agents/metis/security-policy.md` — HAT case records formally classified as SENSITIVE; local-first principle and per-agent internet permissions documented; code security rules for SQL, filesystem, external commands, and Shiny UI captured
...
- Agent Context Documents: hat-dashboard-context.md and mlm-course-context.md added to agents/software-engineer/

### New files
| File | Purpose |
|---|---|
| R/mod_agents.R | Agent Output page module — new tab under "More" |
| agents/software-engineer/hat-dashboard-context.md | HAT Dashboard RAG context for Software Engineer agent |
| agents/software-engineer/mlm-course-context.md | MLM Course context for Software Engineer agent |
...
- Agent context lookup uses a slug-to-filesystem convention: `agents/<slug>/<project-context>.md` — see ADR-009
...
First deployable version of the Metis Dashboard. All pages existed in scaffold or stub form. The SQLite backend was established and the primary data actions (library scan, meeting import, transcript workflow, news storage) were wired. The app loads from a double-click launcher with no RStudio required.
...
- Double-click launcher (launch_metis.bat)
