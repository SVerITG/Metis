...
**Decision**: Resolve context documents via the path `agents/<agent-slug>/<project-slug>-context.md`, with a fallback to `agents/<agent-slug>/README.md` if no project-specific file exists.

**Rationale**: The `agents/` directory is already the canonical location for all agent files. Keeping context documents there (rather than in the project folder or in the database) means agents own their own context. The slug-based naming convention requires no registry — any file matching the pattern is automatically discoverable. The fallback to README.md ensures the Invoke modal always shows something useful even when a project-specific context file has not yet been written.

**Consequences**: When a new project is added and assigned to an agent, a corresponding context file should be created in `agents/<agent-slug>/`. The Invoke modal will silently fall back to README.md if it is missing — no error, but possibly less useful context.
...
**Decision**: Grant the news-aggregator agent internet access in the per-agent security table of `agents/metis/security-policy.md`. All other v6.0 agents (ux-engineer, epidemiologist) retain the default no-internet permission.
...
**Decision**: Every substantive agent output is written to two places: (1) a markdown file in `outputs/reviews/{agent-slug}/{YYYY-MM-DD}_{task-slug}.md`, and (2) a row in the `agent_runs` SQLite table via `log_agent_run()`. Quick factual questions and status checks are exempt from recording (DB log only for those).
