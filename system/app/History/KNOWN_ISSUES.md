...
**Next step**: Add `lessons.json` schema documentation to `agents/software-engineer/` or a `docs/` folder.
...
**Description**: The master security policy lives at `agents/metis/security-policy.md`. Four agent system prompts (dashboard-engineer, meeting-memory, librarian, news-radar) contain relevant excerpts. If the master policy is updated, those excerpts must be manually reviewed and updated. There is no automated sync or validation step.
...
**Root cause**: Agents were created without a corresponding row being added to the per-agent security table in `agents/metis/security-policy.md`.
...
**Files changed**: agents/metis/security-policy.md
