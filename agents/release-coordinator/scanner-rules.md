...
| N2 | `the researcher` | literal | Family name |
| N3 | `<user>` | literal | Lowercase username form |
| N4 | `S\.V\.` | regex | Initials with escaped dots |
| N5 | `the researcher V` | literal | Partial name (name + initial) |
...
| E2 | `<user>@` | literal | Email address prefix |
...
| ID | Pattern | Type | Notes |
|---|---|---|---|
| I1 | `your institution` | literal | Full institutional name |
...
| ID | Pattern | Type | Notes |
|---|---|---|---|
| P1 | `/mnt/c/Users/<user>` | literal | WSL path |
| P2 | `C:\\Users\\<user>` | literal | Windows backslash path |
| P3 | `C:/Users/<user>` | literal | Windows forward-slash path |
| P4 | `OneDrive` | literal | OneDrive folder name |
...
| ID | Pattern | Type | Notes |
|---|---|---|---|
| D1 | `metis\.sqlite` combined with path containing `<user>` or `OneDrive` | compound | SQLite path pointing to personal data location |
...
| `the researcher` in a test fixture `name = "the researcher"` or equivalent placeholder context | Clearly not personal data |
...
| 🔴 BLOCK | N2 (`the researcher`), E1, E2 (email), I1 (institution full name) | Hard stop. Do not push. Report file:line. |
