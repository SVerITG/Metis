...
- `the researcher` — family name
- `<user>` — username form
- `S\.V\.` — initials
- `the researcher V` — partial name
...
### Institution
- `your institution` — full phrase (flag as-is)
- `\bITG\b` — abbreviation, ONLY when NOT inside a variable name, import path, or code comment

### Local paths
- `/mnt/c/Users/<user>` — WSL path
- `C:\\Users\\<user>` — Windows backslash path
- `C:/Users/<user>` — Windows forward-slash path
- `OneDrive` — OneDrive folder name
...
- Any SQLite path pointing to a personal data location (match: `metis\.sqlite` combined with a path containing `<user>` or `OneDrive`)
...
- `the researcher` appearing inside a code comment or test fixture where it is clearly used as a placeholder value (e.g., `name = "the researcher"` in a test)
...
ISS="C:\\Users\\<user>\\OneDrive\\Documents\\7. Software\\Research Cortex\\metis\\system\\install\\installer\\metis-setup.iss"
