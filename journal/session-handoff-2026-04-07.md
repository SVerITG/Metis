...
File: `system/mcp-server/src/metis_mcp/tools/self_improvement.py`
...
```
system/mcp-server/.venv/bin/metis-mcp
```
...
```json
{
  "mcpServers": {
    "metis-pkm": {
      "command": "/mnt/c/Users/<user>/OneDrive/Documents/7. Software/PKM/metis/system/mcp-server/.venv/bin/metis-mcp",
      "env": {
        "METIS_PKM_ROOT": "/mnt/c/Users/<user>/OneDrive/Documents/7. Software/PKM/metis"
      }
    }
  }
}
...
If the username on the other computer is different, adjust `<user>` in both paths.

**Step 2 — Restart Claude Code** on the other computer.

**Step 3 — Verify in terminal:**
```bash
/mnt/c/Users/<user>/OneDrive\ -\ ITG/Documents/7.\ Software/PKM/metis/system/mcp-server/.venv/bin/metis-mcp --help
```
...
On Windows: `C:\Users\<user>\AppData\Roaming\Claude\claude_desktop_config.json`
...
```json
{
  "mcpServers": {
    "metis-pkm": {
      "command": "C:\\Users\\<user>\\OneDrive\\Documents\\7. Software\\PKM\\metis\\08_system\\mcp-server\\.venv\\Scripts\\python.exe",
      "args": ["-m", "metis_mcp.server"],
      "env": {
        "METIS_PKM_ROOT": "C:\\Users\\<user>\\OneDrive\\Documents\\7. Software\\PKM\\metis"
      }
    }
  }
}
...
2. Location: `C:\Users\<user>\OneDrive\Documents\7. Software\PKM\metis\07_outputs\apps\metis-dashboard\launch_monia.bat`
3. Name it: `Research Cortex`
4. Right-click the shortcut → Properties → Change Icon
5. Browse to: `C:\Users\<user>\OneDrive\Documents\7. Software\PKM\Metis_github.png`
...
# Then:
magick convert "C:\Users\<user>\OneDrive\Documents\7. Software\PKM\Metis_github.png" -resize 256x256 "C:\Users\<user>\OneDrive\Documents\7. Software\PKM\metis_icon.ico"
```
...
5. Click **Save** — stored in `outputs/meetings/` and logged in SQLite

The Meeting Memory agent then structures it:
```
/meeting-memory structure the MLM course meeting from [date]
```
It will read the raw file, produce a structured note with action items, decisions, and follow-ups, and save it to `outputs/reviews/meeting-memory/`.
...
- Save the audio file to `outputs/meetings/audio/`
...
1. Open Claude Code terminal in the metis folder:
   ```
   cd "/mnt/c/Users/<user>/OneDrive/Documents/7. Software/PKM/metis"
   ```
2. Run `/metis_status` to see what's open, blocked, or overdue
3. Drop files into `inbox/` and run `/metis` to route them
4. All agent outputs go to `outputs/reviews/{agent-slug}/`
