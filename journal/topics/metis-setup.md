...
For step-by-step instructions see `system/SETUP.md`.
...
| How to create | Run `bash setup.sh` from `system/mcp-server/` |
...
WSL equivalent: `/mnt/c/Users/<user>/AppData/Roaming/Claude/claude_desktop_config.json`
...
| Source location | `system/mcp-server/src/metis_mcp/` |
| Tool registration | Each `tools/*.py` imports `app` from `server.py` and uses `@app.tool()` |
| DB path | `system/app/data/metis.sqlite` |
